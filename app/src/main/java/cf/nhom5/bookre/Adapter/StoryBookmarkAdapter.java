...
public class StoryBookmarkAdapter extends RecyclerView.Adapter<StoryBookmarkAdapter.StoryViewHolder>{
    ArrayList<Story> data_holder;

    public StoryBookmarkAdapter(ArrayList<Story> data_holder) {
        this.data_holder = data_holder;
    }

    @NonNull
    @Override
    public StoryBookmarkAdapter.StoryViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.row_story_bookmark, parent, false);
        return new StoryBookmarkAdapter.StoryViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull StoryBookmarkAdapter.StoryViewHolder holder, int position) {
